...
export async function createChat(client) {
  try {
    ...
    const createdChat = await Chat.create(chatName, client.user.id);
...
export async function renameChat(client) {
  try {
    const { chatId, newChatName } = client.req.params;
    await Chat.renameById(chatId, newChatName);
    client.updateRes("statusCode", 201);
...
export async function searchChat(client) {
  try {
    const { chatName } = client.req.params;
    const chat = await db.select("chat").where({ name: chatName }).run();
...
export async function joinChat(client) {
  try {
    const { chatId } = client.req.params;
    await Chat.joinById(chatId, client.user.id);
    client.updateRes("statusCode", 200);
...
export async function exitChat(client) {
  try {
    const { chatId } = client.req.params;
    await Chat.exitById(chatId, client.user.id);
    client.updateRes("statusCode", 200);
