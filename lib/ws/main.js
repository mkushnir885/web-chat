...
import db from '../database/init-db.js';
...
eventEmitter.on('USER_ONLINE', async (ws, data) => {
  ...
  const rawMessages = await db.select('chat_message').run();
  const messages = await Promise.all(rawMessages.map(async (message) => {
    const [{ nickname }] = await db
      ...
      .run();
    return { author: nickname, timestamp: message.timestamp, body: message.body };
  }));
  ws.send(JSON.stringify({ event: 'CHAT_HISTORY', messages }), { binary: false });
});
